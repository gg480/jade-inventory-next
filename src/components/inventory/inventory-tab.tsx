...
function InventoryTab() {
  ...
  const [saleForm, setSaleForm] = useState({ actualPrice: 0, channel: 'store', saleDate: new Date().toISOString().slice(0, 10), note: '', customerId: '', paymentMethod: 'cash', paymentStatus: 'paid' });
  const [scanSku, setScanSku] = useState('');
  const [scanLoading, setScanLoading] = useState(false);
  const [printLabelItem, setPrintLabelItem] = useState<any>(null);
  const [showScanner, setShowScanner] = useState(false);

  // BarcodeScanner loaded via next/dynamic (ssr: false)
  ...
  // Listen for escape key to close detail drawer
  useEffect(() => {
    const handler = () => { setDetailOpen(false); setDetailItem(null); };
  ...
  async function handleSale() {
    ...
    try {
      const salePayload: any = { itemId: saleDialog.item.id, actualPrice: saleForm.actualPrice, channel: saleForm.channel, saleDate: saleForm.saleDate, note: saleForm.note, paymentMethod: saleForm.paymentMethod, paymentStatus: saleForm.paymentStatus };
  ...
  async function handleScanSku() {
    ...
    try {
      ...
      if (item.status === 'in_stock') {
        ...
        setSaleForm({ actualPrice: item.sellingPrice, channel: 'store', saleDate: new Date().toISOString().slice(0, 10), note: '', customerId: '', paymentMethod: 'cash', paymentStatus: 'paid' });
        setScanSku('');
      } else {
  ...
  return (
    <div className="space-y-6">
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-44">
                              ...
                              {item.status === 'in_stock' && (
                                <DropdownMenuItem onClick={(e) => { e.stopPropagation(); setSaleDialog({ open: true, item }); setSaleForm({ actualPrice: item.sellingPrice, channel: 'store', saleDate: new Date().toISOString().slice(0, 10), note: '', customerId: '', paymentMethod: 'cash', paymentStatus: 'paid' }); }}>
                ...
                <div className="flex items-center gap-1 flex-wrap" onClick={e => e.stopPropagation()}>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-40">
                      ...
                      {item.status === 'in_stock' && (
                        <DropdownMenuItem onClick={() => { setSaleDialog({ open: true, item }); setSaleForm({ actualPrice: item.sellingPrice, channel: 'store', saleDate: new Date().toISOString().slice(0, 10), note: '', customerId: '', paymentMethod: 'cash', paymentStatus: 'paid' }); }}>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1"><Label>付款状态</Label>
      ...
      <ItemDetailDrawer
        item={detailItem}
        open={detailOpen}
        onOpenChange={(open) => { setDetailOpen(open); if (!open) setDetailItem(null); }}
        onEdit={(item) => setEditItemId(item.id)}
        onSell={(item) => {
          setSaleDialog({ open: true, item });
          setSaleForm({ actualPrice: item.sellingPrice, channel: 'store', saleDate: new Date().toISOString().slice(0, 10), note: '', customerId: '', paymentMethod: 'cash', paymentStatus: 'paid' });
        }}
        onReturn={(item) => setReturnConfirmItem({ open: true, item })}
        onPrintLabel={(item) => setPrintLabelItem(item)}
      />
    </div>
  );
}
